-- Publisher permission has been received. Make every active AE notebook
-- viewable and transferable by a verified portal account while retaining the
-- private GitHub source, acknowledgment notice, checksums, and audit records.

create or replace function public.has_verified_portal_account()
returns boolean
language sql
stable
security definer
set search_path = ''
as $$
  select exists (
    select 1
    from auth.users as account
    where account.id = auth.uid()
      and account.email_confirmed_at is not null
  );
$$;

create table public.ae_notebook_view_audit (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users(id) on delete restrict,
  account_email text not null,
  notebook_slug text not null references public.ae_notebook_files(slug) on delete restrict,
  ae_number text not null,
  source_filename text not null,
  source_sha256 text not null,
  notebook_version text not null,
  status text not null default 'authorized'
    check (status in ('authorized', 'delivered', 'failed')),
  requested_at timestamptz not null default now(),
  delivered_at timestamptz,
  failure_code text
);

create index ae_notebook_view_audit_user_requested_idx
on public.ae_notebook_view_audit (user_id, requested_at desc);

alter table public.ae_notebook_view_audit enable row level security;
revoke all on table public.ae_notebook_view_audit from anon, authenticated;

create or replace function public.get_ae_delivery_capabilities()
returns table (
  eligible boolean,
  colab_enabled boolean,
  download_enabled boolean,
  notice_version text,
  notice_text text
)
language sql
stable
security definer
set search_path = ''
as $$
  select
    public.has_verified_portal_account(),
    settings.colab_enabled and public.has_verified_portal_account(),
    settings.download_enabled and public.has_verified_portal_account(),
    settings.notice_version,
    settings.notice_text
  from public.ae_delivery_settings as settings
  where settings.singleton;
$$;

create or replace function public.prepare_ae_notebook_view(p_slug text)
returns table (
  audit_id uuid,
  github_path text,
  source_filename text,
  source_sha256 text,
  notebook_version text
)
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_notebook public.ae_notebook_files;
  v_user auth.users;
  v_audit_id uuid;
begin
  if not public.has_verified_portal_account() then
    raise exception 'A verified portal account is required.';
  end if;

  select * into v_notebook
  from public.ae_notebook_files
  where slug = p_slug and active;

  if v_notebook.slug is null then
    raise exception 'The protected notebook is not available.';
  end if;

  select * into v_user from auth.users where id = auth.uid();

  insert into public.ae_notebook_view_audit (
    user_id, account_email, notebook_slug, ae_number, source_filename,
    source_sha256, notebook_version
  ) values (
    v_user.id, lower(v_user.email), v_notebook.slug, v_notebook.ae_number,
    v_notebook.source_filename, v_notebook.source_sha256,
    v_notebook.notebook_version
  )
  returning id into v_audit_id;

  return query
  select v_audit_id, v_notebook.github_path, v_notebook.source_filename,
         v_notebook.source_sha256, v_notebook.notebook_version;
end;
$$;

create or replace function public.finalize_ae_notebook_view(
  p_audit_id uuid,
  p_status text,
  p_failure_code text default null
)
returns void
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_status text := lower(trim(coalesce(p_status, '')));
begin
  if auth.uid() is null then
    raise exception 'Authentication is required.';
  end if;

  if v_status not in ('delivered', 'failed') then
    raise exception 'Unsupported view status.';
  end if;

  update public.ae_notebook_view_audit
  set status = v_status,
      delivered_at = case when v_status = 'delivered' then now() else null end,
      failure_code = case
        when v_status = 'failed' then nullif(trim(coalesce(p_failure_code, '')), '')
        else null
      end
  where id = p_audit_id
    and user_id = auth.uid()
    and status = 'authorized';

  if not found then
    raise exception 'The authorized notebook view record was not found.';
  end if;
end;
$$;

create or replace function public.prepare_ae_notebook_delivery(
  p_slug text,
  p_action text,
  p_notice_version text
)
returns table (
  audit_id uuid,
  github_path text,
  source_filename text,
  source_sha256 text,
  notebook_version text
)
language plpgsql
security definer
set search_path = ''
as $$
declare
  v_action text := lower(trim(coalesce(p_action, '')));
  v_settings public.ae_delivery_settings;
  v_notebook public.ae_notebook_files;
  v_user auth.users;
  v_audit_id uuid;
begin
  if not public.has_verified_portal_account() then
    raise exception 'A verified portal account is required.';
  end if;

  if v_action not in ('colab', 'download') then
    raise exception 'Unsupported notebook delivery action.';
  end if;

  select * into v_settings
  from public.ae_delivery_settings
  where singleton;

  if v_settings.singleton is null
     or (v_action = 'colab' and not v_settings.colab_enabled)
     or (v_action = 'download' and not v_settings.download_enabled) then
    raise exception 'This notebook delivery option is not enabled.';
  end if;

  if p_notice_version is distinct from v_settings.notice_version then
    raise exception 'The access notice has changed. Review it and acknowledge it again.';
  end if;

  select * into v_notebook
  from public.ae_notebook_files
  where slug = p_slug and active;

  if v_notebook.slug is null then
    raise exception 'The protected notebook is not available.';
  end if;

  select * into v_user from auth.users where id = auth.uid();

  insert into public.ae_delivery_audit (
    user_id, account_email, notebook_slug, ae_number, source_filename,
    source_sha256, notebook_version, notice_version, action
  ) values (
    v_user.id, lower(v_user.email), v_notebook.slug, v_notebook.ae_number,
    v_notebook.source_filename, v_notebook.source_sha256,
    v_notebook.notebook_version, v_settings.notice_version, v_action
  )
  returning id into v_audit_id;

  return query
  select v_audit_id, v_notebook.github_path, v_notebook.source_filename,
         v_notebook.source_sha256, v_notebook.notebook_version;
end;
$$;

update public.ae_delivery_settings
set colab_enabled = true,
    download_enabled = true,
    review_test_enabled = false,
    notice_version = 'publisher-approved-2026-07',
    permission_reference = 'Written publisher permission received by the portal owner on 2026-07-29',
    approved_at = now(),
    approved_by = (
      select account.id
      from auth.users as account
      where lower(account.email) = 'yjin@usc.edu'
      limit 1
    )
where singleton;

revoke all on function public.has_verified_portal_account() from public;
revoke all on function public.get_ae_delivery_capabilities() from public;
revoke all on function public.prepare_ae_notebook_view(text) from public;
revoke all on function public.finalize_ae_notebook_view(uuid, text, text) from public;
revoke all on function public.prepare_ae_notebook_delivery(text, text, text) from public;

grant execute on function public.has_verified_portal_account() to authenticated;
grant execute on function public.get_ae_delivery_capabilities() to authenticated;
grant execute on function public.prepare_ae_notebook_view(text) to authenticated;
grant execute on function public.finalize_ae_notebook_view(uuid, text, text) to authenticated;
grant execute on function public.prepare_ae_notebook_delivery(text, text, text) to authenticated;

comment on table public.ae_notebook_view_audit is
  'Audit trail for verified-account browser access to protected AE notebooks.';
comment on function public.prepare_ae_notebook_view(text) is
  'Authorizes and records one verified-account browser view of an active AE notebook.';
